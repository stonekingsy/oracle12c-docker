...
--------
Build the image:
`docker build --force-rm=true --no-cache=true --shm-size=1G -t stonekingsy/oracle12c-ee .`

Running
-------

On first run, the database will be provisioned. Using a basic command like `docker run stonekingsy/oracle12c-ee` will set defaults for everything and the database will not be persisted if you shut it down. You will also need to monitor the output to get the database password. This isn't ideal, so a more complete run command would look like:
